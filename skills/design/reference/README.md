...
- **Inline placeholders:** to mix static and dynamic text in ONE element, type
  ...
  cell), then `bind_variable` the element as usual. At generation time give each
  token its own compound key `variableName::token` (e.g.
  `"greeting::first_name":"Jane"`) instead of a single whole-field string — each
  `{{token}}` is replaced in place. Token names are letters, digits, and
  underscore (no spaces, periods, or hyphens); `page`/`pages` are reserved; a
  variable name must not contain `::`. You cannot style individual tokens (style
  runs don't cross the API). Prefer this over splitting a sentence into several
